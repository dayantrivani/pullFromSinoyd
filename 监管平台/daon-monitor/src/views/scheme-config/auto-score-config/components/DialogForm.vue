      ...
      <div class="basic-info">
        ...
        <a-form layout="inline" :form="form">
          ...
          <a-form-item>
            ...
            <a-input-number :min=0 :step=1 style="width: 120px" v-decorator="[
              'operationPoints',
              {
                initialValue: 0,
                rules: [
...
} from '@/api/modules/auto-score-config.js';
...
export default {
  ...
  data() {
    return {
      ...
      row: {},
      spinning: false,
      content: '若所设置聘用专业运维公司分值+自动计分总分达到100分,则手动计分将无效',
      spanArr: [],
      position: 0,
      count: 0,
      columns: [
    ...
    onSave() {
      this.form.validateFields((err, fieldsValue) => {
        const setValue = {
          ...
          'operationPoints': fieldsValue.operationPoints,
        ...
        let ruleDetails = [];
        if (this.itemList.length !== 0) {
          if (this.itemList[this.itemList.length - 1].totalPoints + this.itemModel.operationPoints > 100) {
            this.$message.warning('聘用专业运维公司分值 + 自动计分总分超过100分', 3);
            return;
          }
          autoPoints = this.itemList[this.itemList.length - 1].totalPoints;
          diyPoints = 100 - autoPoints - this.itemModel.operationPoints;
          // this.itemList.pop();
