      ...
      <tbody style="width: 100%;display: block;">
        ...
        <tr v-for="field in form"
          ...
          <th style="width: 90px;" class="nowrap">
            <span v-if="field.type === 'suit_comparison'">比对因子<br></span>
            {{field.label}}
            <span v-if="field.label2"><br>{{field.label2}}</span>
            <span v-if="field.label3"><br>{{field.label3}}</span>
            <span v-if="field.label4"><br>{{field.label4}}</span>
            <span v-if="field.label5"><br>{{field.label5}}</span>
          </th>
          <td v-if="field.type === 'attachment' || field.type === 'photo'" :class="cellClass">
            <div class="file-list" v-for="item in bucket[field.id]" :key="item">
              <div class="box-inline">{{ item.split('/')[item.split('/').length - 1].split('?')[0] }}</div>
              <div class="box-inline">
                <a-icon type="search" title="查看图片" @click="toBig(item)"/>
              ...
              <div class="box-inline">
                <!-- <el-popover placement="top" width="200" @show="getDownloadSrc(item)">
                  ...
                  <a-icon type="arrow-down" title="下载" @click="downLoadIcon(item)" />
        ...
        <tr v-else-if="field.label && field.children && field.children.length > 0">
          <th>{{field.label}}</th>
          <td class="nopad">
            <template v-if="!field.only_group">
