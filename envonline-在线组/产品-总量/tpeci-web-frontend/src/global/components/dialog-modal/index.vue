<template>
  <el-dialog
    ...
    <template v-if="!scrollbar">
      <!-- 主体内容区 -->
      <div ref="maxScrollbar" :class="contentClass">
        <slot></slot>
      </div>
    </template>
    <el-scrollbar v-else ref="scrollbar" :view="view" class="hide-x" :native="false" :noresize="false">
    ...
    setRespStyle() {
      this.$nextTick(() => {
        ...
        if (width >= 80) {
          ...
          const h = `calc(70vh - ${elHeadFoot}px)`;
          elBody.style.maxHeight = h;
          if (this.scrollbar) { this.$refs.scrollbar.$el.style.height = h; }
        }
