<template>
  ...
  <div class="mod-upload-file" v-loading="loading">
    <el-upload
      ref="addFile"
      action=""
      v-show="!(fileList.length > 0 && countLimit)"
      :multiple="false"
      :auto-upload="true"
      :file-list="fileList"
      ...
      :disabled="disabled"
    >
      <!-- <el-button type="text" class="pad-l-0">添加附件</el-button> -->
      <el-button
        ...
        >
          <div class="item-type">
            <svg class="icon" aria-hidden="true">
              <use v-if="item.status !== 'ready'" :xlink:href="`#${getIcon(item[props.name])}`" />
              <use v-else :xlink:href="`#${getIcon(item.name)}`" />
...
export default {
  ...
  methods: {
    ...
    sendUpload() {
      this.loading = true;
      this.$http.post(`res/document/upload/?objectId=${this.id}`, this.formData).then((response) => {
        if (response.status === 200) {
