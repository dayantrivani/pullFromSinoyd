...
export default {
  ...
  data() {
    return {
      ...
      labelModel: '',   // 输入框显示值，默认值不能为空，故放一个空格字符串
      valueModel: '',   // 实际请求传值
      treeOptions: [],
      once: false,
    };
  ...
  watch: {
    value: {
      handler(val) {
        ...
        this.initHandler();
        this.$nextTick(() => {
          if (this.$refs.tree.currentNode) this.$refs.tree.currentNode.node.isCurrent = false;
        });
      },
      immediate: true,
    },
    options: {
      handler(val) {
        ...
        this.$nextTick(() => {
          this.treeOptions = this.handleParentData(cloneDeep(val), 0);
        });
        this.initHandler();
      },
      immediate: true,
      ...
      if (iNum < this.field.length - 1) {
        arr.forEach((item) => {
          ...
          if (item.children.length) {
            const num = iNum + 1;
            item.children = this.handleParentData(item.children, num);
          }
        });
      }
      return arr;
