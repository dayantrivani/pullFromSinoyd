<template>
  <div class="fill-h">
    ...
    <dialog-form ref="dialogForm" title="报警处理基本信息" @getItemList="getItemList"/>
  ...
  methods: {
    ...
    getAlarmTypeList() {           // 获取报警类型列表
      ...
      this.$http.post('alarm/alarmType/code?page=1&rows=1000000', {
        alarmTypeCodes: ['AlarmOffLine', 'AlarmDataDuplication', 'AlarmValveCloseHasCou', 'AlarmOverProofHour'],
      }).then((response) => {
        if (response.status === 200 && response.data.success) {
    ...
    onDownload() {                 // 下载数据
      ...
      this.$http.post('alarm/alarmRecord/export', {
        ...
        const fileName = `报警处理${new Date().toLocaleDateString()}.xls`;
