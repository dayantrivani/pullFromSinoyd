<template>
  ...
  <div class="fill-h" style="overflow: hidden; background-color: #F5F7FA;">
    <el-row :gutter="10" class="fill-h fn-flex mar-t-xs" v-resize>
      <el-col style="min-width: 280px;margin-right: 10px;background-color: #FFF;" v-loading="ploading">
        <!-- 按钮组 -->
        <el-form class="mar-x-sm mar-t-sm" style="width: 255px;">
          <el-form-item>
            <el-input
              :disabled="true"
              v-model="cardNumber"
            ...
            ></el-input>
            <el-button class="mar-l-xs" type="primary" icon="fas fa-address-card" @click="onReadCard">读 卡</el-button>
          </el-form-item>

          <el-form-item>
            <el-input
              style="max-width:255px;"
              :value="cardNumber ? '用户卡' : ''"
              :disabled="true"
              placeholder="读卡获取卡类型">
            </el-input>
            <!-- <el-select
            ...
            >
              <el-option
                v-for="(item, key) in $enum.get('IC_CARD_TYPE')"
                :key="key"
                :label="item.label"
                :value="item.value">
              </el-option>
          ...
          <el-form-item>
            <el-input
              style="max-width:255px;"
              :value="$enum.getLabel('IC_CARD_STATUS', cardStatus)"
              :disabled="true"
              placeholder="读卡获取卡状态">
            </el-input>
            <!-- <el-select
            ...
            >
              <el-option
                v-for="(item, key) in $enum.get('IC_CARD_STATUS')"
                :key="key"
                :label="item.label"
                :value="item.value">
              </el-option>
          ...
          <el-form-item>
            ...
            <!-- <el-button type="primary" class="mar-l-xs" icon="fas fa-check" @click="onQueryCheck">确 认</el-button> -->
          </el-form-item>

          <el-form-item>
            <el-button
              :disabled="!(cardStatus === 1)"
              type="primary"
              style="width: 100%;"
              icon="fas fa-check"
              title="有效的用户卡才可充值"
...
export default {
  ...
  data() {
    return {
      ...
      itemModel: {                  // 查询条件
        portId: '',                 // 许可证id
        factorList: [],             // 企业名称、站点名称
        depositYear: '',            // 年份
      },
      dItemModel: {                 // 本年度已充值 | 查询条件
        portId: '',                 // 站点
        depositType: '',            // 1 远程充值 2 IC卡充值
        factorList: [],             // 企业名称、站点名称
        depositYear: '',            // 年份
      },
      cardNumber: '',               // 卡号
  ...
  methods: {
    ...
    onQueryCheck() {                 // 确认按钮
      ...
      this.dItemModel.portId = this.itemModel.portId;
      this.getFactorList();
    },
    init() {
    ...
    getMessage(msg) {
      ...
      if (data.Order === 'selectCard') {
        if (data.Success) {
          this.cardNumber = data.Data.cardNum;     // webSocket 获取卡号
          // this.cardStatus = data.Data.cardStatus;  // webSocket 获取卡状态
          // this.cardType = data.Data.cardType;      // webSocket 获取卡类型
        } else {
          this.$message.error(data.Message);
        }
      }
      // 充值
      if (data.Order === 'disCharge') {
        if (data.Success) {
    ...
    getDetailsItemList() {
      ...
      this.$http.post('tpeci/iCCardDepositRecord/recordDetail', this.dItemModel, {
        params: {
          page,
          rows,
          sort: 'createDate-',
    ...
    getCardInfo(val) {                // 通过webScoket获取卡号后,调该接口
      this.ploading = true;
      this.$http.get(`tpeci/iCUserCard/${val}`).then((response) => {
        if (response.status === 200 && response.data.success) {
          this.portName = response.data.data || [];
          this.itemModel.portId = response.data.data.portId;
          this.cardStatus = response.data.data.cardStatus;
        } else {
          this.$message.warning({
            message: response.data.msg,
            duration: 3000,
      ...
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        this.$http.post('tpeci/iCCardDepositRecord', {
          ...
          if (response.status === 200 && response.data.success) {
            ...
            //   showClose: true,
            // });
            this.serialNumber = response.data.data.rid;
            this.getItemList();
            // 写入卡中的数据
            const params = this.itemList.filter(item => item.thisTimeDeposit
              && item.thisTimeDeposit !== '0').map(item => ({
              Tstamp: +(this.newItemModel.depositYear),
              SerialNumber: this.serialNumber,
              FactorCode: item.factorCode,
              RechargeDischarge: item.thisTimeDeposit,
              Amount: item.thisTimeDeposit,
            }));
            if (params != null && params.length > 0) {
              // 监听WebSocket是否处于关闭状态
              if (this.socket.readyState === WebSocket.OPEN) {
                this.dtoSend.Order = 'disCharge';
