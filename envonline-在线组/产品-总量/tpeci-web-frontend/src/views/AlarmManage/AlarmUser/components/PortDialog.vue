<template>
  ...
  >
    ...
    <el-table
      ...
      <el-table-column property="portType" label="排口类型" show-overflow-tooltip>
        <template slot-scope="{ row }">
          {{ getPortTypeName(row.portType, row.childPortType) }}
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    beforeClose() {
      this.queryPortForm.portTypes = [];
      this.$refs.selectTreeMod.labelModel = '';
    ...
    getPortTypeName(portTypeId, childPortTypeId) {
      let name = '';
      const portType = this.$enum.get('PORT_TYPE')[portTypeId];
      if (!portType) return '';
      if (portType.children) {
        name = portType.children[childPortTypeId].dictName;
      } else {
        name = portType.dictName;
      }
      return name;
    },
  },
