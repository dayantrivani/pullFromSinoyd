...
const state = {
  ...
  // 企业管理上传附件默认跳转路由
  uploadPage: 'UPSecureFourColorMap',
  // 附件上传页面信息
  uploadPageInfo: '',
  // 当前企业跳转记录
  PSHistory: '',
  ...
  SectionCode: [],                            // 断面编码 ?
  PortType: [],                               // 排口类型
  WaterPortTypeList: [],                      // 废水排口类型
  GasPortTypeList: [],                        // 废气排口类型
  RainPortTypeList: [],                       // 雨水排口类型
...
const actions = {
  // 获取选择框下拉数据 params格式为 { codeValue: key }
  ...
  // 获取废水、废气子类型排口类型
  async GET_CHILd_PORTTYPE({ state, commit }, params = {}) {
    let val = state[params.codeValue];
    if (val.length === 0) {
      await Vue.http.get(`sys/enumCode/child/PortType/${params.type}`).then((response) => {
        const obj = {};
        obj.list = response.data;
        obj.params = params;
        val = response.data || [];
...
const mutations = {
  ...
  setUploadPage(state, link) {
    state.uploadPage = link;
  },
  setActiveInfo(state, row) {
    state.activeInfo = row;
  },
  setUploadInfo(state, row) {
    state.uploadInfo = row;
  },
  setUploadPageInfo(state, row) {
    state.uploadPageInfo = row;
  },
  setPSHistory(state, obj) {
    state.PSHistory = obj;
  },
  setUploadHistory(state, obj) {
    state.UploadHistory = obj;
  },
  // 设置选择框下拉数据
