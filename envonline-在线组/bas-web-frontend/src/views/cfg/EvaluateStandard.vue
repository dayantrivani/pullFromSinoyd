<template>
    ...
    <div slot="header">
      <el-form inline :model="queryForm">
        <el-form-item label="标准名称：">
          <el-input v-model="queryForm.name" clearable placeholder="标准名称" style="width: 150px"></el-input>
        </el-form-item>
        <el-form-item label="类型：">
          <el-select v-model="queryForm.evaluationType">
            <el-option label="全部" value=""></el-option>
            <el-option v-for="(item, key) in $hp.evalTypeList" :key="key" :label="item" :value="key"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
  ...
  <!-- 弹框表单 -->
    <dialog-modal ref="dialog" size="mini" :title="`${isEdit ? '编辑' : '新增'}评价标准信息`">
        ...
        <el-form ref="form" label-position="right" label-width="100px" :model="itemModel" :rules="rules">
          ...
          <el-form-item label="类型：" prop="evaluationType">
            <el-select v-model="itemModel.evaluationType">
              <el-option
                v-for="(item, key) in $hp.evalTypeList"
                :key="key"
                :label="item"
                :value="key - 0">
              </el-option>
...
import EvaluateSetting from '@/views/cfg/CommonDialog/EvaluateSettingDialog';
