        ...
        <el-tab-pane label="排口配置" lazy>
          <el-form inline :model="queryForms" class="query-form-box mar-t-xs">
            <el-form-item label="行政区域：">
              <select-tree-lazy placeholder="行政区域" v-model="queryForms.regionCodeId" />
            </el-form-item>
            ...
            <el-form-item label="排口类型：">
              <el-select placeholder="排口类型" v-model="queryForms.portType" clearable>
                <el-option v-for="(item, key) in $hp.PortTypeList" :key="key" :label="item" :value="key">
                </el-option>
              </el-select>
            </el-form-item>
          ...
          <el-table
            ...
            <el-table-column property="regionName" label="行政区域" min-width="80" show-overflow-tooltip>
            </el-table-column>
            ...
            <el-table-column property="attentionDegreeName" label="关注程度" min-width="80" show-overflow-tooltip>
            </el-table-column>
            <el-table-column property="dgiMn" label="MN" min-width="180" show-overflow-tooltip></el-table-column>
            <el-table-column label="排口类型" min-width="80" show-overflow-tooltip>
              <template slot-scope="{ row }">
                {{$enum.portType[row.portType]}}
              </template>
            </el-table-column>
          </el-table>
          <!-- 分页 -->
...
export default {
  name: 'RolePort',
  components: { RoleTabPan, TabPanDialog, SelectTreeLazy },
  data() {
    return {
      loading: false,                 // 界面加载
      treeList: [],
  ...
  methods: {
    ...
    ROLE_PORT_DELETE(body) { // 删除角色对应站点
