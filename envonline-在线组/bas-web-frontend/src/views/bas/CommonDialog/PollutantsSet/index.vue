<template>
  ...
  <!-- 修改设置污染物 -->
    ...
    <survey-dialog ref="surveySecondDialog" @fillTableData="fillTableData"/>
    <factory-config-dialog ref="factoryConfigDialog"></factory-config-dialog>
  </div>
</template>
...
import FactoryConfigDialog from './components/FactoryConfigDialog';
...
export default {
  ...
  methods: {
    ...
    onClickConfig(row) {
      this.$refs.factoryConfigDialog.openDialog({ ...row, portType: this.listType });
    },
    beforeCloseDialog() {
      this.$emit('close');
    },
    ...
    getItemList() {
      ...
      this.$http.get('bas/portInfoFactors', {
        params: { page, rows, portId: this.id },
      }).then((response) => {
