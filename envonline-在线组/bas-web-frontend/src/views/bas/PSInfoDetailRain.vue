<template>
  ...
  <el-card fill>
    ...
    <dialog-modal ref="dialog" :title="`${isEdit ? '修改' : '新增'}雨水排口信息`" @closed="onDialogClose">
            ...
            <el-col :span="12">
              <el-form-item label="排口类型：">
                <el-select v-model="itemModel.waterPortType">
                  <el-option
                    v-for="(item, key) in options.RainPortTypeList"
                    :key="key"
                    :label="item.dictName"
  ...
  <!-- 雨水点位附件 -->
    <dialog-modal ref="uploadDialog" title="雨水点位附件">
      <Upload :id="portId"/>
    </dialog-modal>
  ...
  methods: {
    ...
    getEmptyItem() {
      this.portModel = {
        ...
        portName: '',                 // 排口名称
        portShortName: '',            // 排口简称
        portType: 5,                  // 排口类型 1:废水，2:废气 5:雨水
        isOutput: 0,                  // 进出口
        portCode: '',                 // 排口编号
        status: 1,                    // 状态
        contactMan: '',               // 联系人
      ...
      return {
        ...
        waterPortType: 1,             // 1：一拖二 2：雨水排口 3：清下水排口
    ...
    getDictItem() {
      ...
      this.$store.dispatch('psInfo/GET_CHILd_PORTTYPE', {
        codeValue: 'RainPortTypeList',
        type: 'Rain',
      }).then((val) => {
        this.options.RainPortTypeList = val;
      });
