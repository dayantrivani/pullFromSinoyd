  ...
  <dialog-modal ref="dialog" title="排口配置" class="defealt-dialog">
    <el-form inline :model="queryForm">
      ...
      <el-form-item label="关键字：">
        <el-input v-model="queryForm.key" placeholder="企业名称，排口名称，MN号" style="width: 220px"></el-input>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" icon="fas fa-search" @click="getItemList">查 询</el-button>
      </el-form-item>
    </el-form>
    <el-tabs style="height: calc(100% - 45px);"  v-model="tabPortType" @tab-click="onTabClick">
      <el-tab-pane
...
export default {
  ...
  methods: {
    ...
    getItemList() {
      this.getPortTypeList(this.treeData[0].portType).then((response) => {
        this.$set(this.treeData[0], 'children', [...this.buildTree(this.getPortChildren(response))]);
        this.mixRegionAndPort(this.treeData[0].children);
      });
    ...
    onTabClick(tab) {
      this.getPortTypeList(tab.name).then((response) => {
        this.$set(this.treeData[tab.index], 'children', [...this.buildTree(this.getPortChildren(response))]);
        this.mixRegionAndPort(this.treeData[tab.index].children);
      });
    },
