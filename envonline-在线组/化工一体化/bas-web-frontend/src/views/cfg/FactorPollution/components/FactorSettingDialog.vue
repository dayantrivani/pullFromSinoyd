    ...
    <!-- 因子列表 -->
      ...
      <el-table
        ...
        <el-table-column label="所属类型" show-overflow-tooltip>
          <template slot-scope="scope">
            {{ $enum.CategoryList[scope.row.category] }}
          </template>
        </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      factorItem: {}, // 父级点击对象
      factorsTypeItems: [
        'Avg', 'Rtd', 'Cou', 'Max', 'Min', 'ZsMax', 'ZsMin', 'ZsAvg', 'ZsCou', 'ZsRtd',
      ], // 值类型下拉框枚举
      rules: {                // 编辑表单验证规则
        factorCode: [
  ...
  methods: {
    ...
    // 单击新增回调
    onClickAdd() {
      ['factorName', 'factorCode', 'category'].forEach((ele) => {
        this.$set(this.itemModel, ele, this.factorItem[ele]);
      });
