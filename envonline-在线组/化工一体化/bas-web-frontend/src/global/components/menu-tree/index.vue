...
export default {
  ...
  computed: {
    ...
    randerData() {
      // appendNode
      const rander = {};
      const data = [...this.appendNode, ...this.itemList];
      this.$set(rander, this.propsMerge.value, this.rootKey);
      this.$set(rander, this.propsMerge.label, this.title);
      this.$set(rander, this.propsMerge.children, data);
      return this.lazy ? data : [rander];
    },
  },
  watch: {
    collapse(val) {
        ...
        if (this.option.length) {
          ...
          this.$http.get(url, { params: { ...this.queryForm, ...this.queryParams } }).then((response) => {
            ...
            if (this.highlightNode) {
              this.$nextTick(() => {
                this.$refs.tree.setCurrentKey(this.highlightNode || null);
              });
            }
            this.loading = false;
