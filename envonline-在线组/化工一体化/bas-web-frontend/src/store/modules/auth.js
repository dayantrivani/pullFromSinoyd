...
import infoListConfig from '@/config/info-list-config';
...
const state = {
  ...
  userRoles: window.sessionStorage.getItem('userRoles')
    ? JSON.parse(window.sessionStorage.getItem('userRoles'))
    : [],
  // 用户可路由
  addRouter: window.sessionStorage.getItem('addRouter')
    ? JSON.parse(window.sessionStorage.getItem('addRouter'))
    : [],
  // 登录信息
...
const getters = {
  ...
  psNavMenu: state => (state.userRoles.includes('super')
    ? infoListConfig
    : infoListConfig.filter(v => v.label === '环保信息')),
};
...
const actions = {
  ...
  GetUserRoles({ commit }) {
    return new Promise((resolve) => {
      http.get(`sinoyd-frame/sys/users/${state.user.loginID}/roles`)
        .then((response) => {
          if (response.data.success) {
            commit('SetUserRoles', response.data.data.map(v => v.roleCode));
          }
          resolve();
        });
    });
...
const mutations = {
  ...
  SetUserMenu(state, menu) {
    ...
    if (menus.length) {
      ...
      menus.map((v) => {
        if (v.moduleUrl === 'PSList') {
          v.label = state.user.userType === 'ent' ? '企业档案' : v.label;
        }
