...
-->
  ...
  <div class="mod-upload-file">
    ...
    -->
    <el-upload ref="addFile"
               action=""
               multiple
               :limit="limit"
               :auto-upload="false"
               v-show="!(fileList.length === limit) && btn.includes('add')"
               :file-list="fileList"
               :on-remove="onRemove"
               :on-change="onChange"
      ...
      <el-button type="text"
                 class="pad-l-0"
                 @click="onClickUpload">添加附件</el-button>
    </el-upload>
    <!-- 文件列表展示区 -->
    <el-row :gutter="5"
            style="margin-top: -5px"
            v-if="fileList.length !== 0">
      <el-col class="mar-t-xs"
        ...
        <div :class="{ 'file-item': 1, 'fn-flex': 1,
          ...
          <div class="item-type">
            <svg class="icon"
                 aria-hidden="true">
              <use v-if="item.status !== 'ready'"
                   :xlink:href="`#${getIcon(item[props.name])}`"></use>
              <use v-else
                   :xlink:href="`#${getIcon(item.name)}`"></use>
              <!-- <use :xlink:href="`#${item.fileType.substr(1)}`"></use> -->
            </svg>
          </div>
          <div class="item-name flex-1">
            <div v-if="item.status !== 'ready'"
                 :title="item[props.name]">{{ item[props.name] }}</div>
            <div v-else
                 :title="item.name">{{ item.name }}</div>
          </div>
          <div class="item-option">
            <div class="btn-down"
                 :title="item.status === 'ready' ? '需上传后才能下载' : '下载'"
                 v-show="btn.includes('down')"
                 @click="onClickDown(item)">
              <i class="el-icon-download"></i>
            </div>
            <div v-show="btn.includes('remove')"
                 class="btn-close"
                 title="删除"
    ...
    <div v-else-if="!btn.includes('add')">暂无数据</div>
  </div>
</template>
...
export default {
  ...
  watch: {
    ...
    // id(val) { if (val) this.getItemList(); }, // 新增附件上传后获取id会获取附件列表导致清空附件列表
  },
  methods: {
    getIcon: getFileIcon,
    // 父组价调用该方法 -- 上传附件
    ...
    uploadQuery(formatData) {
      this.$http.post(`res/document/upload/?objectId=${this.id}`, formatData).then((response) => {
        if (response.status === 200) {
          ...
          if (this._events['update-success']) {
            this.getItemList().then((res) => {
              this.$emit('update-success', true, res);
            }, () => {
              this.$emit('update-success', true);
            });
          }
