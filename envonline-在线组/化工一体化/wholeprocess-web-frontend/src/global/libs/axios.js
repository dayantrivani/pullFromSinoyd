...
function checkExpireToken({ response, config }) {
  ...
  return new Promise((resolve, reject) => {
    if (config.url.indexOf('refresh-token') === -1 && response.data.msg.includes('授权已过期')) {
...
Axios.interceptors.request.use(
  (cfg) => {
    ...
    // if (cfg.url.indexOf('wp/') === 0) {
    //   cfg.url = `sinoyd-wp/${cfg.url}`;
    // }
    ...
    // if (cfg.url.indexOf('orgduty/') === 0) {
    //     cfg.url = `sinoyd-wrybase/${cfg.url}`;
    //   }

    // if (cfg.url.indexOf('wholeprocess/') === 0) {
    //   cfg.url = `sinoyd-wholeprocess/${cfg.url}`;
    // }
