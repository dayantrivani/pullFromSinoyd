<template>
  <el-card fill>
    ...
    <el-table border
      ...
      <el-table-column property="projectDescription"
                       label="项目说明"
                       min-width="100"
                       show-overflow-tooltip>
      </el-table-column>
      <el-table-column property="budgetCost"
                       label="预算费用（万元）"
                       min-width="150"
                       show-overflow-tooltip>
      </el-table-column>
      <el-table-column property="budgetIdentifyPeople"
                       label="预算识别人"
                       min-width="150"
                       show-overflow-tooltip>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    onClickExport() {
      ...
      this.$http.post('wholeprocess/safetyProductionCostBudget/export', this.queryForm, {
        responseType: 'arraybuffer',
      }).then((response) => {
        const fileName = `费用预算${new Date().toLocaleDateString()}.xls`;
