<!-- 弹窗表格选择企业 -->
<template>
  <div>
    <el-input v-model="labelModel"
              readonly
              class="z-pointer"
              :size="size">
      <el-button slot="append"
                 :size="size"
                 @click="openDialog()">
        <i class="fas fa-th-list"></i>
      </el-button>
    </el-input>
    <!-- 弹窗 -->
    <dialog-modal ref="dialog"
                  :title="`选择排查责任人`"
                  @close="closeDialog"
                  append-to-body
                  scrollbar>
      <el-form inline
               :model="queryForm">
        <el-form-item label="责任人名称：">
          <el-input v-model="queryForm.name"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary"
                     icon="fas fa-search"
                     @click="getItemList">查 询</el-button>
        </el-form-item>
      </el-form>
      <!-- 列表 -->
      <el-table border
                highlight-current-row
                height="calc(70vh - 115px)"
                v-loading="loading"
                :data="itemList"
                :row-class-name="rowClassName"
                @select="onChangeCheck"
                @select-all="onChangeCheckAll"
                @selection-change="onChangeSelected">
        <el-table-column align="center"
                         label="No."
                         width="50"
                         type="index"
                         :index="index"></el-table-column>
        <el-table-column align="center"
                         label="操作"
                         width="76">
          <template slot-scope="scope">
            <el-button icon="el-icon-check"
                       type="success"
                       @click="onClickCheck(scope.row)">确认</el-button>
          </template>
        </el-table-column>
        <el-table-column property="deptmentName"
                         label="所属部门/车间"
                         min-width="180"
                         show-overflow-tooltip></el-table-column>
        <el-table-column property="name"
                         label="责任人名称"
                         min-width="180"
                         show-overflow-tooltip></el-table-column>
        <el-table-column property="mobilePhone"
                         label="手机号码"
                         show-overflow-tooltip></el-table-column>
        <el-table-column property="jobLevel"
                         label="职级职位"
                         show-overflow-tooltip></el-table-column>
      </el-table>
      <!-- 分页 -->
      <paging-query :pager="pager"
                    @query="getItemList" />
    </dialog-modal>
  </div>
</template>

<script>
import { mapGetters } from 'vuex';
import { queryList } from '@/global/libs/mixins';

export default {
  mixins: [queryList],
  name: 'SelectTable',
  // 设置绑定参数
  props: {
    indexName: {
      default: '',
    },
    label: {
      default: '',
    },
  },
  watch: {
    label(val) {
      if (!val) {
        this.labelModel = '';
      }
    },
  },
  computed: mapGetters('auth', ['userType', 'orgGuid']),
  created() {
    this.labelModel = this.label ? this.label : '';
    if (this.userType === 'ent') {
      this.queryForm.orgId = this.orgGuid;
    }
  },
  data() {
    return {
      loading: false,
      queryForm: {
        name: '', // 责任人名称
        deptName: '', // 车间名称
        orgId: '', // 企业id
        sort: 'name',
      },
      labelModel: '',
      isAuto: false,
      size: 'small',
      url: 'wholeprocess/personInfo',
    };
  },
  methods: {
    openDialog() {
      this.getItemList();
      this.$refs.dialog.openDialog();
    },
    closeDialog() {
      this.$refs.dialog.closeDialog();
    },
    onClickCheck(row) {
      this.labelModel = row.name;
      this.$emit('selectSingleData', row, this.indexName);
      this.$refs.dialog.closeDialog();
    },
  },
};
</script>
