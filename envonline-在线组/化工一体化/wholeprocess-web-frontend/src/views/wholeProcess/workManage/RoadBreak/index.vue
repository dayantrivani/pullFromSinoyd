        ...
        <el-form inline
          ...
          <el-row>
            <el-col :span="3"
              ...
              断路作业开展中：{{count.inProgress}}个
            </el-col>
            <el-col :span="3"
                    style="color:#67C23A;font-size: 15px;font-weight: 550;">
              断路作业完成数：{{count.completed}}个
            </el-col>
...
export default {
  ...
  methods: {
    ...
    onClickExport() {
      ...
      this.$http.post('wholeprocess/roadBreakWork/export', this.queryForm, {
        responseType: 'arraybuffer',
      }).then((response) => {
        // const fileName = decodeURI(response.headers['content-disposition']).split('=')[1];
        const fileName = `断路作业${new Date().getTime()}.xls`;
