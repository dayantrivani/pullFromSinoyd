        ...
        <el-form inline
          ...
          <el-row>
            <el-col :span="4"
              ...
              高处作业开展中：{{count.inProgress}}个
            </el-col>
            <el-col :span="4"
                    style="color:#67C23A;font-size: 15px;font-weight: 550;">
              高处作业完成数：{{count.completed}}个
            </el-col>
          ...
          <el-form :model="item"
            ...
            <el-row style="height:30px;">
              ...
              <el-col style=";margin-top:5px;"
                      :span="6">
                <el-form-item label="作业级别：">
                  <span v-if="item.workLevel === 1">一级</span>
                  <span v-if="item.workLevel === 2">二级</span>
                  <span v-if="item.workLevel === 3">三级</span>
                  <span v-if="item.workLevel === 4">四级</span>
                </el-form-item>
              </el-col>
            ...
            <el-row>
              ...
              <el-col :span="6">
                <el-form-item label="作业高度：">
                  <span>{{item.workHeight ? `${item.workHeight}m` : ''}}</span>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
...
export default {
  ...
  methods: {
    ...
    onClickExport() {
      ...
      this.$http.post('wholeprocess/heightWork/export', this.queryForm, {
        responseType: 'arraybuffer',
      }).then((response) => {
        // const fileName = decodeURI(response.headers['content-disposition']).split('=')[1];
        const fileName = `高处作业${new Date().getTime()}.xls`;
