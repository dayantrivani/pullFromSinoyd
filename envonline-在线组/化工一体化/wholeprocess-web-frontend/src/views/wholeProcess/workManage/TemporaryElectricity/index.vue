        ...
        <el-form inline
          ...
          <el-row>
            <el-col :span="4"
              ...
              临时用电作业开展中：{{count.inProgress}}个
            </el-col>
            <el-col :span="3"
                    style="color:#67C23A;font-size: 15px;font-weight: 550;">
              临时用电作业完成数：{{count.completed}}个
            </el-col>
            ...
            <el-row>
              ...
              <el-col :span="6">
                <el-form-item label="电源接入点：">
                  <span>{{item.powerAccessPoint}}</span>
                </el-form-item>
              </el-col>
              <el-col :span="6">
                <el-form-item label="工作电压：">
                  <span>{{item.workVoltage}}</span>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
...
export default {
  name: 'TemporaryElectricityManage',
  mixins: [
    queryList,
  ],
  components: {
  ...
  methods: {
    ...
    onClickExport() {
      ...
      this.$http.post('wholeprocess/temporaryElectricityWork/export', this.queryForm, {
        responseType: 'arraybuffer',
      }).then((response) => {
        // const fileName = decodeURI(response.headers['content-disposition']).split('=')[1];
        const fileName = `临时用电作业${new Date().getTime()}.xls`;
