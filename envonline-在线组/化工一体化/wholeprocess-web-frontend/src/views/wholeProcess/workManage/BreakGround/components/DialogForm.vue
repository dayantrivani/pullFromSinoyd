          ...
          <el-form-item label="涉及的其他特殊作业：">
            <el-checkbox-group v-model="specialWork">
              <span v-for="(item, index) in $enum.WORKTYPE" :key="index">
                <el-col :span="6">
                  <!-- 32动土作业 -->
                  <el-checkbox v-if="item.value !== 32"
        ...
        <el-col :span="24">
          <el-form-item label="危害辨识：">
            <el-checkbox-group v-model="selectHazards">
              <span v-for="item in dict.BreakGroundHazardIdentification" :key="item.rowGuid">
                <el-col :span="6">
      ...
      <el-row>
        <el-col :span="24">
          <el-form-item label="作业内容：">
            <el-input type="textarea" v-model="itemModel.workContent"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
...
export default {
  ...
  data() {
    return {
      isAuto: false,
      url: 'wholeprocess/breakGroundWork',
      specialWork: [],    // 特殊作业
      selectHazards: [],
      itemModel: this.getEmptyItem(),
      dict: {
        BreakGroundHazardIdentification: [], // 危险辨识
      },
      // 表单验证
      rules: {
        deptId: [
  ...
  methods: {
    ...
    getEmptyItem() {
      return {
        ...
        makerId: '',
        permitCode: `DTZY-${this.$tools.formatDate(new Date(), 'YmdHi')}${Math.floor(Math.random() * 1000)}`,
        workStatus: 0,
        deptId: '',
        applicantName: '',
        safetyGuardian: '',
    ...
    onSaveItem() {
      this.$refs.form.validate((valid) => {
        ...
        this.selectHazards.forEach((id) => {
          const item = {
            hazardId: id,
            hazardName: this.dict.BreakGroundHazardIdentification.find(e => e.rowGuid === id)
              && this.dict.BreakGroundHazardIdentification.find(e => e.rowGuid === id).dictName,
          };
          hazards.push(item);
        });
