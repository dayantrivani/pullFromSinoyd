        ...
        <el-form inline
          ...
          <el-row>
            <el-col :span="3"
              ...
              动火作业开展中：{{count.inProgress}}个
            </el-col>
            <el-col :span="3"
                    style="color:#67C23A;font-size: 15px;font-weight: 550;">
              动火作业完成数：{{count.completed}}个
            </el-col>
...
export default {
  ...
  methods: {
    ...
    onClickExport() {
      ...
      this.$http.post('wholeprocess/fireWork/export', this.queryForm, {
        responseType: 'arraybuffer',
      }).then((response) => {
        // const fileName = decodeURI(response.headers['content-disposition']).split('=')[1];
        const fileName = `动火作业${new Date().getTime()}.xls`;
