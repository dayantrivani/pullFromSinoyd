  ...
  methods: {
    ...
    getEmptyItem() {
      return {
        ...
        makerId: '',
        permitCode: `DHZY-${this.$tools.formatDate(new Date(), 'YmdHi')}${Math.floor(Math.random() * 1000)}`,
        workStatus: 0,
        deptId: '',
        applicantName: '',
        isFlammableExplosive: false,
        isExternal: false,
        externalCompany: '',
        externalContactName: '',
    ...
    onSaveItem() {
      this.$refs.form.validate((valid) => {
        ...
        this.selectHazards.forEach((id) => {
          const item = {
            hazardId: id,
            hazardName: this.dict.FireHazardIdentification.find(e => e.rowGuid === id)
              && this.dict.FireHazardIdentification.find(e => e.rowGuid === id).dictName,
          };
          hazards.push(item);
        });
