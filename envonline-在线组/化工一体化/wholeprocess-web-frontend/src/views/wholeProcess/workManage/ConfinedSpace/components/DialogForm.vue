<template>
  <dialog-modal ref="dialog"
    ...
    <el-form ref="form"
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="受限空间名称：" prop="confinedSpaceName">
            <el-input v-model="itemModel.confinedSpaceName"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="受限空间原有介质名称：" prop="originalMediumName">
  ...
  data() {
    return {
      ...
      rules: {
        ...
        originalMediumName: [
          { required: true, message: '请输入受限空间内原有介质名称', trigger: ['blur', 'change'] },
        ],
        beginDate: [
  ...
  methods: {
    ...
    getEmptyItem() {
      return {
        ...
        makerId: '',
        permitCode: `SXZY-${this.$tools.formatDate(new Date(), 'YmdHi')}${Math.floor(Math.random() * 1000)}`,
        workStatus: 0,
        deptId: '',
        applicantName: '',
        confinedSpaceName: '',
    ...
    onSaveItem() {
      this.$refs.form.validate((valid) => {
        ...
        this.selectHazards.forEach((id) => {
          const item = {
            hazardId: id,
            hazardName: this.dict.ConfinedSpaceHazardIdentification.find(e => e.rowGuid === id)
              && this.dict.ConfinedSpaceHazardIdentification.find(e => e.rowGuid === id).dictName,
          };
          hazards.push(item);
        });
