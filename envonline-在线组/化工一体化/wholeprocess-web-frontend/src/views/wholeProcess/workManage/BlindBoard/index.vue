        ...
        <el-form inline
          ...
          <el-row>
            <el-col :span="4"
              ...
              盲板抽堵作业开展中：{{count.inProgress}}个
            </el-col>
            <el-col :span="3"
                    style="color:#67C23A;font-size: 15px;font-weight: 550;">
              盲板抽堵作业完成数：{{count.completed}}个
            </el-col>
            ...
            <el-row>
              ...
              <el-col :span="6">
                <el-form-item label="实施时间：">
                  {{item.blockingDate ? $tools.formatDate(item.blockingDate, 'Y-m-d') : ''}} 至
                  {{item.pullingDate ? $tools.formatDate(item.pullingDate,'Y-m-d') : ''}}
                </el-form-item>
              </el-col>
              <el-col :span="6">
                <el-form-item label="设备管道名称：">
                  <span>{{item.equipmentPipelineName}}</span>
                </el-form-item>
              </el-col>
              <el-col :span="6">
            ...
            <el-row>
              ...
              <el-col :span="6">
                <el-form-item label="审核时间：">
                  {{item.auditDate ? $tools.formatDate(item.auditDate,'Y-m-d') : ''}}
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
...
export default {
  ...
  methods: {
    ...
    onClickExport() {
      ...
      this.$http.post('wholeprocess/blindBoardWork/export', this.queryForm, {
        responseType: 'arraybuffer',
      }).then((response) => {
        // const fileName = decodeURI(response.headers['content-disposition']).split('=')[1];
        const fileName = `盲板抽堵作业${new Date().getTime()}.xls`;
