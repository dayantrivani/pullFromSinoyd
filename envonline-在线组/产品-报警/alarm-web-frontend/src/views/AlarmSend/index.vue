<template>
  ...
  <el-card class="title-card" type="title">
    ...
    <el-table
      ...
      <el-table-column align="center" prop="waterStorageStatus" min-width="67px" label="操作" show-overflow-tooltip>
    ...
    <dialog-modal ref="dialog" title="发送方案配置" :before-close="beforeClose" size="mini">
      <el-form :rules="rules" :model="configForm" ref="form" label-width="120px" class="item-space pad-x-sm">
        ...
        <el-form-item label="时间间隔(min)：" prop="sendTimeSpan">
          <!-- 正整数 -->
          <el-input-number
            v-model="configForm.sendTimeSpan"
...
export default {
  ...
  methods: {
    ...
    getAlarmTypeList() { // 获取报警类型数据
      this.$http.get('alarm/alarmType?page=1&rows=1000000').then((response) => {
        if (response.data.success) {
