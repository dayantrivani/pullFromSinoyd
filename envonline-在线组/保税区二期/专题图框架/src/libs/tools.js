 ...
 * @param {any} data            存储数据
 */
const encryption = false;  // 加密数据

export const setLocalStorage = (key, data) => {
  const jsonStr = JSON.stringify(data);
  const result = window.encodeURIComponent(jsonStr);
  window.localStorage.setItem(key, encryption ? window.btoa(result) : result);
};

export const getLocalStorage = (key) => {
  const data = window.localStorage.getItem(key);
  if (data) {
    const atobStr = encryption ? window.atob(data) : data;
    const result = window.decodeURIComponent(atobStr);
...
export const setSessionStorage = (key, data) => {
  const jsonStr = JSON.stringify(data);
  const result = window.encodeURIComponent(jsonStr);
  window.sessionStorage.setItem(key, encryption ? window.btoa(result) : result);
};
