...
import { analyzeResp, getToken, removeToken } from '@/libs/util';
import {
  login, getUserNav, refreshToken, getUserList, logout,
} from '@/api/auth';
    ...
    return new Promise((resolve, reject) => {
      ...
      login(data).then((response) => {
        if (response.status === 200 && response.data.code !== 500) {
          // setLocalStorage('canRefresh', params.remember ? 1 : 0);
...
const mutations = {
  ...
  SetUserInfo(state, user) {
    ...
    // setSessionStorage('userInfo', state.userInfo);
  },
  // 设置用户导航
  SetUserMenu(state, menu) {
    ...
    // setSessionStorage('userMenu', state.userMenu);
