...
(function (factory) {
  ...
  L.Proj.Projection = L.Class.extend({
    initialize(code, def, bounds) {
      const isP4 = L.Proj._isProj4Obj(code);
  ...
  L.Proj.CRS = L.Class.extend({
    ...
    initialize(a, b, c) {
      ...
      if (this.options.origin) {
        this.transformation =					new L.Transformation(1, -this.options.origin[0],
  ...
  L.Proj.ImageOverlay = L.ImageOverlay.extend({
    ...
    _animateZoom(event) {
      ...
      const northWest = L.point(this._projectedBounds.min.x, this._projectedBounds.max.y);
      const offset = this._projectedToNewLayerPoint(northWest, event.zoom, event.center);
    ...
    _projectedToNewLayerPoint(point, zoom, center) {
      ...
      const newTopLeft = this._map.project(center, zoom)._subtract(viewHalf)._round();
      const topLeft = newTopLeft.add(this._map._getMapPanePos());
