...
const json_parse = (function () {
  ...
  let stack;
  // The stack, for controlling nesting.

  let container;
  // The current container object or array
  ...
  const string = {   // The actions for string tokens
    go() {
      state = 'ok';
    },
    firstokey() {
  ...
  const number = {   // The actions for number tokens
    go() {
      state = 'ok';
    },
    ovalue() {
  ...
  const action = {
    ...
    '{': {
      ...
      ovalue() {
        stack.push({ container, state: 'ocomma', key });
        container = {};
        state = 'firstokey';
      },
      firstavalue() {
        stack.push({ container, state: 'acomma' });
        container = {};
        state = 'firstokey';
      },
      avalue() {
  ...
  function debackslashify(text) {
    ...
    return text.replace(/\\(?:u(.{4})|([^u]))/g, (a, b, c) => (b ? String.fromCharCode(parseInt(b, 16)) : escapes[c]));
  }
