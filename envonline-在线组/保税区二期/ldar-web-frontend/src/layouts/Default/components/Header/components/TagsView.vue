...
<template>
  <div class="tags-view">
    <!-- 操作按钮 -->
    <div class="btn-scoll">
      <button :class="{ 'is-active': isActive({name: 'home' }) }" @click="$router.replace('/')">
    ...
    <div ref="tagsc" class="container">
      <ul class="m-scoll" :style="{ left: `-${scollOffset}px` }">
        <router-link
          ...
          @contextmenu.prevent.native="openMenu(tag, $event)">{{ getTitle(tag.name) }}
...
export default {
  ...
  computed: {
    // 显示的标签对象
    visitedViews() {
      return this.$store.state.tagsView.visitedViews.filter(v => v.name !== 'home');
    },
    // 侧栏菜单
    sideMenu() {
  ...
  methods: {
    getTitle(name) {
      ...
      const fn = (data) => {
        data.forEach((item) => {
          if (!item.children.length && item.moduleCode === name) {
            title = item.label;
            return;
          }
