...
const state = {
  ...
  level: 1,
  userList: [],
  amcsType: window.sessionStorage.getItem('amcsType')
    ? +(window.sessionStorage.getItem('amcsType'))
    : 1,
...
const mutations = {
  ...
  SetAmcsType(state, type) {
    state.amcsType = type;
    window.sessionStorage.setItem('amcsType', type.toString());
  },
  Logout(state) {
