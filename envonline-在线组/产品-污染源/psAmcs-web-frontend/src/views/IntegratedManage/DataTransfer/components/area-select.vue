  ...
  methods: {
    ...
    initHandler() {
      ...
      this.$nextTick(() => {
        ...
        // 初始化显示
        this.labelModel = temp.length !== 0 ? temp.map(n => n.areaName).join(',') : '';
        // 设置默认选中
        this.$refs.tree.setCurrentKey(this.value);
      });
    },
    // 切换选项
    onNodeCheck(item, node) {
      const labelModel = node.checkedNodes.map(n => n.areaName).join(',');
      const valueModel = node.checkedKeys;
