...
export default {
  ...
  methods: {
    openDialog(row) {
      ...
      this.options.yAxis[0].name = `浓度${this.itemModel.factorName.match(/\(.+?\)/g).reverse()[0]}`;
      this.options.series.markLine.data[0].yAxis = this.itemModel.upperValue;
      this.options.series.markLine.data[1].yAxis = this.itemModel.lowValue;
      this.getDataCurve();
    },
    drawEcharts() {
