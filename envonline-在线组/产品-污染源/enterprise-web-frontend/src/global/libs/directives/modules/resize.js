...
import { getElemAttrVal, createElem } from '@/global/libs/tools';
...
export default {
  bind(el) {
    ...
    const onResizeEvent = (colL, colR) => {
      ...
      colL.appendChild(createElem(lineClassName, 'span'));
      ...
      onChild(colL, lineClassName, (e) => {
        ...
        const boxWidth = getElemAttrVal(box, 'width');         // 获取容器宽度
        const leftWidth = getElemAttrVal(colL, 'width');       // 获取主元素(左)原始宽度
        const rightWidth = getElemAttrVal(colR, 'width');      // 获取响应元素(右)原始宽度
