  ...
  if (response.status === 422) {
    ...
    Message({
      message: (response.data && response.data.msg)
        ? response.data.msg.replace(/java.lang.NullPointerException:/, '')
        : '操作有误或者服务器错误',
...
/*
  正则匹配
  如: ([吸光度]-[b])/[k]/[取样体积]*[稀释倍数]
  取值: ['吸光度', 'b', 'k', '取样体积', '稀释倍数']
*/
const getRegString = (str) => {
  const regex = /\[[^[|\]]*]/g;
  const regString = str.match(regex);
  const result = regString ? regString.map(e => e.substring(1, e.length - 1)) : [];
  return result;
};
