  ...
  methods: {
    ...
    getAlarmTypeList() {
      ...
      this.$http.post('alarm/alarmType/code',
        { alarmTypeCodes: ['AlarmForFullSample', 'AlarmWarnForFullSample'] }).then((response) => {
        if (response.data.success) {
          this.alarmTypeList = response.data.rows || response.data.data || response.data;
          const allId = this.alarmTypeList.map(n => n.id).join(',');
          this.alarmTypeList.unshift({ alarmTypeName: '全部', id: allId });
          this.$set(this.queryForm, 'alarmTypeIds', allId.split(','));
        } else {
          this.$message({
            type: 'warning',
            message: response.data.msg,
