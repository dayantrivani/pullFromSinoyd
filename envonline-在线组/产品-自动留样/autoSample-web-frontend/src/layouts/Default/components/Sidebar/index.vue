  ...
  <aside>
    ...
    <el-scrollbar class="hide-x" :native="false" :noresize="false">
      <el-menu
        ...
        <menu-item v-for="item in sideMenu" :key="item.moduleCode" :data="item" />
      </el-menu>
    </el-scrollbar>
    <!-- 导航设置对话框 -->
...
export default {
      ...
      if (!this.isCollapse) {
        ...
        menus.forEach((item) => {
          if (this.level >= 1 && item.children.length) {
            // 第一层数据
            tempMenu.push(item.moduleCode);
            item.children.forEach((item1) => {
              if (this.level >= 2 && item1.children.length) {
                // 第二层数据
                tempMenu.push(item1.moduleCode);
                item1.children.forEach((item2) => {
                  if (this.level >= 3 && item2.children.length) {
                    // 第三层数据
                    tempMenu.push(item2.moduleCode);
                  }
                });
              }
            });
  ...
  watch: {
    ...
    $route: function (route) {
      // 当点击第一级导航时收起已展开的节点
      if (this.sideMenu.map(v => v.moduleCode).includes(route.name)) {
        this.closeAllMenu();
      }
    },
  },
  ...
  methods: {
    ...
    closeAllMenu() {
      this.sideMenu.forEach((v) => {
        this.$refs.menu.close(v.moduleCode);
      });
    },
    // 打开侧栏设置
    openSideSet() {
