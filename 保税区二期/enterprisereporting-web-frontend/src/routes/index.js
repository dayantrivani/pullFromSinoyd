...
export const asyncRoutesMap = [
  ...
  },
  {
    ...
    name: 'EntReported',
    meta: { title: '企业报备' },
    component: lazy('EntReported'),
  },
  {
    path: 'EntEnergyReported',
    name: 'EntEnergyReported',
    meta: { title: '企业能耗使用情况上报' },
    component: lazy('EntEnergyReported'),
  },
  {
    path: 'EnergyConsumption',
    name: 'EnergyConsumption',
    meta: { title: '企业蒸汽使用情况上报' },
    component: lazy('EnergyConsumption'),
  },
  {
    path: 'EntUseReported',
    name: 'EntUseReported',
    meta: { title: '企业原材料使用情况上报' },
    component: lazy('EntUseReported'),
  },
];
...
export const routesMap = [
  ...
  },
  {
    ...
    children: [
      {
        path: '',
        redirect: { name: (store.state.auth.addRouter[0] || config.home) },
      },
      ...asyncRoutesMap,
      {
        path: 'Empty',
...
router.beforeEach((to, from, next) => {
  ...
  // 以下页面可以直接跳转
  if (/login|lock|NotFound|404/.test(to.name)) {
    next({ replace: true });
    return;
  }
  // 如果导航菜单中不存在，就跳转到notFont页面；即判断是否有权限
