      ...
      <el-card>
        ...
        <div class="pad-x-sm pad-t-sm" style="height: calc(100% - 40px); overflow: scroll;">
          <ul class="month-box" v-loading="statusLoading">
            <li v-for="(item, index) in reportStatus" :key="index" @click="onClickMonth(item >= 0, index + 1)" :class="{
              disabled: +item === -1,
              'bg-danger': +item === 0,
              'bg-success': +item === 1,
...
export default {
  ...
  data() {
    return {
      ...
      reportStatus: [],
      // 右侧记录
      url: 'enterprise/rawMaterial/monReportStatus',
      isAuto: false,
      // 查询表单绑定变量
      queryForm: {
        belongYear: new Date().getFullYear().toString(),
  ...
  methods: {
    ...
    getReportStatus() {
      ...
      const params = { belongYear: this.belongYear };
      if (['ent', 'Ent'].includes(this.userType)) params.psId = this.orgGuid;
      this.$http.get('enterprise/rawMaterial/calReportStatus', {
        params,
      }).then((response) => {
