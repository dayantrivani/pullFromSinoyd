...
const getters = {
  ...
  sideMenu: (state) => {
    ...
    // if (state.user.userType !== 'gov') {
    //   const items = state.userMenu.find(v => v.moduleCode === 'InformationSecurity');
    //   const result = state.userMenu.filter(v => !['InformationSecurity', 'VideoWworkMonitor'].includes(v.moduleCode));
    //   if (items) {
...
const actions = {
  ...
  LogoutSafety(context, o) {
    ...
    o.vm.$jsonp(`http://${host}/a/exit`, {
      loginName: o.loginInfo.telephone,
      callback: 'callback',
    }).finally(() => {
  ...
  GetUserRoles({ commit }) {
    return new Promise((resolve) => {
      http.get(`sinoyd-frame/sys/users/${state.user.loginID}/roles`)
        .then((response) => {
          if (response.data.success) {
            commit('SetUserRoles', response.data.data.map(v => v.roleName));
          }
          resolve();
        });
    });
