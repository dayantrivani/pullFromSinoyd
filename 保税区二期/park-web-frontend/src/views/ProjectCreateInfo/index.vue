  ...
  <el-card>
    <template slot="header">
      <el-form inline :model="queryForm">
        <el-form-item label="企业名称：">
          <el-select v-model="queryForm.psId" autofocus filterable clearable style="width:150px">
            <el-option v-for="(item, index) in psBaseList" :key="index" :label="item.psName" :value="item.psId">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="项目名称：">
          <el-input v-model="queryForm.projectName" clearable style="width:150px">
    ...
    <div class="mar-x-sm mar-y-xs fn-flex fn-no-wrap">
      ...
      <div class="flex-1 text-right pad-l-sm">
        ...
        <el-button type="primary" icon="fas fa-upload" :disabled="!itemList.length" @click="onClickExport()">导 出
        </el-button>
      </div>
    </div>
    ...
    <el-table border highlight-current-row height="calc(100% - 71px)" v-loading="loading" :data="itemList"
      ...
      <el-table-column property="projectDate" label="项目时间" min-width="100" show-overflow-tooltip>
        <template slot-scope="scope">
          {{ scope.row.projectDate | date }}
        </template>
      </el-table-column>
      <el-table-column property="projectAcceptance" label="是否验收" min-width="100" show-overflow-tooltip>
        <template slot-scope="scope">
          {{ scope.row.projectAcceptance===1?'是':'否' }}
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    getItemList() {
      ...
      this.$http.post(`${this.url}/find`, this.queryForm, {
        params: { page, rows, sort: 'projectDate-' },
      }).then((response) => {
