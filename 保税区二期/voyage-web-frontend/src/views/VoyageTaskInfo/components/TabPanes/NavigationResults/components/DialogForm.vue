...
export default {
  ...
  data() {
    return {
      ...
      formRules: {                              // 表单验证
        ...
        factorCode: [
          { required: true, message: '请选择超标特征污染物', trigger: ['blur', 'change'] },
        ],
        factorValue: [
  ...
  methods: {
    openDialog(row) {               // 打开对话框
      this.$set(this.itemModel, 'taskId', this.mainrow.taskId);
      if (row) {
        this.itemModel = _.cloneDeep(row);
        this.$set(this.itemModel, 'taskId', row.taskId || this.mainrow.taskId);
      }
