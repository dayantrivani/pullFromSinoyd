...
<template>
  <el-card>
    ...
    <el-table
      ...
      <el-table-column fixed property="longitude" label="经度" min-width="90" show-overflow-tooltip></el-table-column>
      <el-table-column fixed property="latitude" label="纬度" min-width="90" show-overflow-tooltip></el-table-column>
      ...
      <el-table-column
        ...
        <template slot-scope="{ row }">
          ...
          <p v-if="row[`${item.factorCode}_alertUpper`] !== '' && isRed(row[item.factorCode])">
            {{ row[`${item.factorCode}_alertUpper`] }}
          </p>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    openDialog() {
      if (this.mainrow.voyageDate) {
        const time = this.mainrow.voyageDate.split(' ')[0];
        this.queryForm.fromTime = `${time} 00:00:00`;
        this.queryForm.toTime = `${time} 23:59:59`;
      }
